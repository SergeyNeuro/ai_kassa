...
class ChangingDishDbCache(database.BaseMenu, CommonCacheDb):
    cache = get_cache_obj(key=config.CACHE_NAME)
    db = DbChoicer(db_name=config.DB_TYPE).choice_changing_dish_obj()
    ...
    async def get_data_by_id(self, node_id: int) -> Union[db_schemas.changing_dish.ChangingDishSchem, None]:
        ...
        data = await self.search_in_cache_or_db_and_save(
            key=f"changing_dish_id:{node_id}",
            db_func=self.db.get_data_by_id,
            schema=db_schemas.changing_dish.ChangingDishSchem,
            live_time=60,
            args=(node_id,)
        )
